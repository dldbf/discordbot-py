...
async def on_message(message):
    ...
    if message.content == f'{PREFIX}샌찬':
        ...
        embed.add_field(name="신체특징", value="머리가 푸른색이다. 염색을 하더라도 보통 사람보다 더 빨리 돌아오는듯 하다, 눈이 푸른색이다 하지만 밝게 보이지는 않는다", inline=False)
        embed.add_field(name="이명", value="불가역, 하늘 대가리", inline=False)
        embed.add_field(name="능력치", value="", inline=False)
   ...
   if message.content == f'{PREFIX}도기':
        ...
        embed.add_field(name="신체특징", value="금빛 머리와 금빛 눈을 지니고 있다, 팔근육이 많다 그래서 그런지 팔뚝이 굵다", inline=False)
        embed.add_field(name="이명", value="판타지 브레이커, 환살", inline=False)
        embed.add_field(name="능력치", value="", inline=False)
    ...
    if message.content == f'{PREFIX}뉴비':  
        ...
        embed.add_field(name="신체특징", value="붉은 머리를 지니고 있다 선천적으로 붉은 눈을 지녔다해서 머리는 염색한거라 한다, 굉장히 잘생겼다, 겉으로는 드러나지 않지만 신체스팩이 괴물이다", inline=False)
        embed.add_field(name="이명", value="능력자학교의 괴물, 호리호리 하지만 우락부락", inline=False)
        embed.add_field(name="능력치", value="", inline=False)
